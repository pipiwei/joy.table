            ...
            for (var i = 0; i < lsItems.length; i++) {
                ...
                for (var j = 0; j < keys.length; j++) {
					...
					*/
                        ...
                        if (attrName == "class" && !checkIsEmpty(attrVal)) {
...
JoyTable.prototype.TableAddRowElement = function TableAddRowElement() {
	...
	for (var i=0; i<cloneTdObjs.length; i++) {
		...
		if (tdName == "check_row") {
		...
		else if (tdObj.hasClass("editable")) {
            if (this.EditSenario == 1) {
                var bindingEditEventObj = {
                    handleEvent: function (e) {
                        if (this.tableClassObj.EditMode) {
                            ...
                            this.tableClassObj.RootObject.find("._active_edit").remove();
                            // gen input object of this column and fill original value
                            var editComponent = $("<input type='text'></input>"); // todo EditSenario=1: should get correct input object from config, now only support text box
                            var dataId = this.rowObj.attr("id");
                            var dataProp = this.colObj.attr("name");
                            editComponent.attr("id", dataId + "_edt");
                            editComponent.attr("name", dataProp);
                            editComponent.addClass("_active_edit");
                            editComponent.val(this.colObj.text());
                            // binding event
                            var bindingEditCompEventObj = {
                                handleEvent: function (e) {
                                    var _editComponent = this.tableClassObj.RootObject.find("._active_edit");
                                    if (e.keyCode === 13 || e.type == "change") { // enter accept changed
                                        console.log("edit cell enter");
                                        var _dataId = this.rowObj.attr("id");
                                        var _dataProp = this.colObj.attr("name");
                                        var _newVal = _editComponent.val();
                            ...
                            // showup input object
                            this.colObj.append(editComponent);
                            editComponent.focus();
                            
                            editComponent[0].addEventListener("keyup", bindingEditCompEventObj, false);
                            editComponent[0].addEventListener("keypress", bindingEditCompEventObj, false);
                            editComponent[0].addEventListener("change", bindingEditCompEventObj, false);
                        }
                    },
                    tableClassObj: this,
                    colObj: tdObj,
                ...
                $(tdObj)[0].addEventListener("dblclick", bindingEditEventObj, false);
            }
		}
	}
